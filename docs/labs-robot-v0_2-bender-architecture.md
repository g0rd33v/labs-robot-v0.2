...
**Scope note:** §5 defines Soul's *runtime contract* only. The full design — the persona dial, the complete Soul Loop and evolution mechanics — is specified in `docs/SOUL.md`; builders implement that document, through this interface. *(Amended 2026-07-31: this note previously pointed at a document called SOUL-MASTER and at an "AEI framing". The owner confirms neither ever existed — no SOUL-MASTER was written, and AEI is expanded and defined nowhere. `SOUL.md` is the design that reference was reaching for; AEI is struck pending a definition, see SOUL.md §12.)*
