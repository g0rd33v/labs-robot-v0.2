...
## Провайдеры: закреплённый порядок вместо sort=latency (2026-09-01)

`provider_sort: "latency"` заменён на `providers: ["DeepInfra",
"Together"]` с включёнными fallbacks. Одно поле, обратимо.

**Две вещи ломались, вторая дороже.**

Сортировка не даёт низкой задержки: замерено напрямую, один и тот же
префикс 6K, gemma-4-31b, два прогона — `sort: latency` дал p50
1487/2148 ms при p95 **18443/3411 ms**, выбирая эндпоинты на 19.5 s и
37.5 s, тогда как закреплённый DeepInfra в том же окне держал
792–2664 ms. Сортировка идёт по устаревшей глобальной статистике.

И разбросанный вызов никогда не прогреет кэш, потому что кэш у каждого
эндпоинта свой. Именно этим и были 32.5% на роут-сиденье — среднее от
случайных попаданий двух подряд в один эндпоинт.

**Замер после (боевой робот, не пробник):**
...
Семидневная база до: cache 32.5%, route p50 2882 ms, p95 9094 ms.

**Поправка к собственному выводу.** Синтетический пробник дал 0% кэша на
шести закреплённых вызовах и я записал «кэш — тупик, его почти никто не
отдаёт». Неверно: пробник гонял одно предложение, повторённое 260 раз,
`max_tokens: 8`, шесть вызовов в одном всплеске. Настоящий префикс — 5K
каталога в cache-stable раскладке, и он копится по ходам. Исходная
гипотеза была верна целиком; cache-stable раскладка speed-транша не была
сломана — ей не на чем было накапливаться.

**Проверено.** 261 тест, clippy чист. Полный `eval --live` НЕ гонялся —
указ 11, только по команде.
