...
//!
//! V1 floor contents per Q17: time/date, self/meta, explicit reminder with
//! parseable time, cancel/list of reminders, help. English + Russian
//! surface patterns (the floor scans surface text; everything it *produces*
//! is English-internal per arch sec 2d).
...
pub fn scan(text: &str, now: DateTime<Local>) -> Option<FloorMatch> {
    ...
    // help
    if matches!(joined.as_str(), "help" | "/help" | "commands" | "/start" | "помощь" | "команды") {
...
fn parse_reminder(
    ...
    if matches!(lower.get(i).map(String::as_str), Some("tomorrow") | Some("завтра")) {
        tomorrow = true;
        i += 1;
    }
    ...
    match lower.get(i).map(String::as_str) {
        ...
        Some("in") | Some("через") if !tomorrow => {
            ...
            let minutes = match unit.as_str() {
                "minute" | "minutes" | "min" | "mins" | "минуту" | "минуты" | "минут" | "мин" => n,
...
mod tests {
    ...
    fn floor_matches_time_self_help_in_both_languages() {
        ...
        assert_eq!(scan("cancel reminder", now()), Some(FloorMatch::CancelReminder));
    }
